...
public class WebClient
{
	...
	private String request(String relativeUrl, HttpArgs args, boolean isGet)
	{
		...
		catch (Throwable ex)
		{
			...
			if (outString != null && outString.indexOf(">") > 0){
				...
				if (!times.isEmpty()){
					Logger.write(relativeUrl, "¬се врем€ выполнени€ запроса: "+Long.toString(swAll.getTime())+"; ¬рем€ получени€ потока данных: "+Long.toString(swRequest.getTime())+"; ¬рем€ обработки сервером запроса: "+Long.toString(times.iterator().next()));
				}
			}
		}
	}
	...
	private static String convertStreamToString(InputStream is)
	{
		...
		sw.stop();
		Logger.write("***ѕреобразование байтов в строку***", "¬рем€: "+Long.toString(sw.getTime()));
		return  outString;
	}
	...
	// actionTypeName - это перечень выбранных видов де€тельности формат строки:
	// <Search><Value>“елефоны</Value><Value>“орты</Value><Value>любой
	// текст</Value></Search>
	...
	// 3 - по попул€рности по возрастанию
	// 4 - по попул€рности по убыванию
			...
			long catalogNodeId, int start, int offset, int orderType)
	{
		...
		sw.stop();
		Logger.write("Parsing items", "¬рем€ парсинга: "+Long.toString(sw.getTime())+";  ол-во элементов:"+Integer.toString(items.size()));
	...
	// ѕример xml описани€ <ItemList><Id>1</Id><Id>2</Id><Id>3</Id></ItemList>
...
/*
	...
	// все сообщени€ которые необходимо показывать в ActionBar и которые ранее не были показаны
	...
	// возвращает кол-во сообщений доступных дл€ показа в ActionBar 
